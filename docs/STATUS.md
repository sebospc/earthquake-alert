...
- **developer-ios:** (25-sep) `ios-client/` rebuilt: `RelayCore` Swift package (GatewayClient for /devices and /status, CoverageTracker with the 5 min restart grace (only when /status does not answer), retrying() with capped backoff + jitter (4xx never retried), AppState.derive, EarthquakeAlert decode), `swift test` in `ios-client/RelayCore` 33/33 on the Mac (incl. contract test against the real gateway, needs node + gateway npm install). App = one SwiftUI screen, iOS 26, `xcodegen generate` then build; `-state <name>` launch arg shows any sample state (DEBUG). Not built yet, waiting for the research: notification mechanism, location, receptor choice, Focus warning, wiring the app to real state.
